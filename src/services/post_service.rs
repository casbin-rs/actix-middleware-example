...
pub fn find_all(req: HttpRequest, pool: &web::Data<Pool>) -> Result<Vec<Post>, ServiceError> {
    ...
    let user =
        User::find_user_by_username(username, &pool.get().unwrap()).map_err(|_| make_error())?;
...
pub fn find_by_id(req: HttpRequest, id: i32, pool: &web::Data<Pool>) -> Result<Post, ServiceError> {
    fn make_error() -> ServiceError {
        ServiceError::new(
            StatusCode::INTERNAL_SERVER_ERROR,
            constants::MESSAGE_CAN_NOT_FIND_USER.to_string(),
